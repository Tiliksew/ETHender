...
const validateEmail=async email=>{
    const Email=await account.findOne({email})
    console.log(Email)
...
if(!user){
    return res.status(404).json({message:"Username not found",succes:false})

    
}
if(user.role!==role){
    return res.status(403).json({message:"Make sure to login to the right endpoint",succes:false})
...
return res.status(200).json({...result,succes:false})
    ...
    return res.status(403).json({message:'Incorrect Password',succes:false})
}
